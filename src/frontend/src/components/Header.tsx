...
function NavLinkItem({
  ...
  ocid,
  onClick,
}: { href: string; label: string; ocid: string; onClick?: () => void }) {
  const state = useRouterState();
  const isActive = state.location.pathname === href;
  return (
    <Link
      to={href}
      data-ocid={ocid}
...
export function Header() {
  ...
  const state = useRouterState();
  const isServicesActive = state.location.pathname.startsWith("/services");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link
            ...
            aria-label="Tru Vacate Cleaning Gold Coast - Home"
          >
          ...
          >
            <NavLinkItem href="/" label="Home" ocid="nav.home_link" />
            ...
            <NavLinkItem href="/about" label="About" ocid="nav.about_link" />
            <NavLinkItem
              href="/contact"
              label="Contact"
          ...
          <div className="hidden md:flex items-center gap-2">
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-md text-sm font-semibold text-white transition-all hover:opacity-90 hover:-translate-y-0.5"
        ...
        <div className="md:hidden border-t border-border bg-white">
          ...
          >
            <Link
              ...
              className="px-3 py-2.5 text-sm font-medium rounded-md text-foreground"
            >
              Home
            </Link>
            <Link
              to="/services"
              onClick={() => setMobileOpen(false)}
              className="px-3 py-2.5 text-sm font-semibold text-primary"
            >
              All Services
            </Link>
            {services.map((s) => (
              <Link
                ...
                to={s.href}
                onClick={() => setMobileOpen(false)}
                className="pl-6 pr-3 py-2 text-sm font-medium rounded-md text-foreground"
              >
                {s.label}
              </Link>
            ))}
